...
ResultSet rs1 = stmt.executeQuery("select * from products where market = 0 and event = 0");
rs1.last();
int oneEvent = rs1.getRow();
request.setAttribute("one",oneEvent);

ResultSet rs2 = stmt.executeQuery("select * from products where market = 0 and event = 1");
rs2.last();
int twoEvent = rs2.getRow();
request.setAttribute("two",twoEvent);

ResultSet rs3 = stmt.executeQuery("select * from products where market = 0 and event = 2");
rs3.last();
int plusEvent = rs3.getRow();
request.setAttribute("plus",plusEvent);

ResultSet rs = stmt.executeQuery("select * from products where market = 0");
rs.last();
...
%>
<jsp:forward page="JSP_FOLDER/UI/module/productshowCU.jsp"></jsp:forward>
