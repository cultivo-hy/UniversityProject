...
<div data-role="page" id="pageone">
  <div data-role="header" data-position="fixed" style="top: 0px; border-top-width: 0px; padding-top: 0px;">
    ...
    <img id="haha" src = "image/CU.png"></img>
      <ul>
  ...
  <div data-role="main" class="ui-content" id = "content" style = "padding-top: 10px;padding-left: 10px;padding-bottom: 10px;padding-right: 10px;">
  	  	<%	
  	  	int max=0;
  	  	if(one%2 == 0)	{	max = (one/2);	}
  	  	else{				max = (one/2)+1;}
		for(int i=0; i<max; i++){
			rs.next();
		%><table class="product"><tr><div>
		 ...
		 <table class="product"><div><td id=left_name><%=name%></td>
		            <% if(flag){ %><td id=right_name><%=rs.getString(2)%></td>
		        	<% 	}
		        	%>
		   </div></tr>
		    <tr><div><td id=left_cost><%=cost%>원</td>
		         	<% if(flag){ %>
